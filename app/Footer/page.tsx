...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#C47E63] text-white border-t mt-auto py-10 px-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-8 rounded-none px-2 md:px-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
          {socials.map((item) => (
            <Button
              key={item.name}
              className="bg-white text-[#C47E63] hover:bg-white/95 hover:underline flex items-center gap-2 rounded-none px-4 py-3 transform-gpu transition duration-200 ease-out hover:scale-105 hover:shadow-lg"
        ...
        <div className="flex flex-col items-start gap-2 text-sm text-white/80 sm:items-end">
          <span>&copy; {new Date().getFullYear()} Tati Montero Yoga.</span>
          <Link href="/politicas" className="hover:underline text-white/80">
